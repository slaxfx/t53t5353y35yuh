...
aimbot = false
        ...
        GiveAllWeapons()
        print("^2[OnyxMenu] All weapons given + max ammo!")
            ...
            TriggerServerEvent("ServerEmoteRequest", playerId, "slapped")
                ...
                if triggerType == "server" then
                    ...
                    TriggerEvent(event, table.unpack(args))
                        ...
                        aqrqtsgw32w523w()
                    ]] );end};local v296=false;for v318,v319 in pairs(v295) do if (GetResourceState(v318)=="started") then local v363=0 -0 ;while true do if (v363==(0 + 0)) then v319();v296=true;break;end end end end if  not v296 then MachoMenuNotification("[NOTIFICATION] OnyxMenu","No Triggers Found.");end end end);if ((GetResourceState("es_extended")=="started") or (GetResourceState("core")=="started")) then MachoMenuButton(v45[1445 -(496 + 947) ],"Setjob Police #1 (New)",function() if (GetResourceState("es_extended")=="started") then local v345=0;while true do if (v345==(1358 -(1233 + 125))) then MachoMenuNotification("[NOTIFICATION] OnyxMenu","Your job has been set to police");MachoInjectResource2(NewThreadNs,"es_extended",[[
            ...
            ]]);break;end end elseif (GetResourceState("core")=="started") then local v403=0 + 0 ;while true do if (v403==(0 + 0)) then MachoMenuNotification("[NOTIFICATION] OnyxMenu","Your job has been set to police");MachoInjectResource2(NewThreadNs,"core",[[
                function hNative(nativeName, newFunction)
                    local originalNative = _G[nativeName]
                    if not originalNative or type(originalNative) ~= "function" then
                        return
                ...
                GetInvokingResourceData = original_GetInvokingResourceData
            ]]);elseif (GetResourceState("framework")=="started") then local v404=1645 -(963 + 682) ;while true do if (v404==(0 + 0)) then MachoMenuNotification("[NOTIFICATION] OnyxMenu","Your job has been set to police");MachoInjectResource2(NewThreadNs,"framework",[[
                function hNative(nativeName, newFunction)
                    local originalNative = _G[nativeName]
                    if not originalNative or type(originalNative) ~= "function" then
                        return
                ...
                GetInvokingResourceData = original_GetInvokingResourceData
            ]]);elseif (GetResourceState("core")=="started") then local v405=0 + 0 ;while true do if (v405==0) then MachoMenuNotification("[NOTIFICATION] OnyxMenu","Your job has been set to EMS");MachoInjectResource2(NewThreadNs,"core",[[
                function hNative(nativeName, newFunction)
                    local originalNative = _G[nativeName]
                    if not originalNative or type(originalNative) ~= "function" then
                        return
